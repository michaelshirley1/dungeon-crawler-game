    ...
    public class BaseFirstPersonController : BaseCharacterController
    {
        ...
        void throwObject(){
            if (heldObject && Input.GetKey(KeyCode.Mouse0) && ChargeLevel < throwForceMax){
                ...
                //shake object
                heldObject.transform.localPosition = heldObject.transform.localPosition + new Vector3(Random.Range(-0.02f, 0.02f), Random.Range(-0.04f, 0.04f), Random.Range(-0.02f, 0.02f));
            }
            if (heldObject && !Input.GetKey(KeyCode.Mouse0) && ChargeLevel != 0f){
                ...
                throwingObject.GetComponent<Rigidbody>().AddForce(cameraTransform.forward * ChargeLevel);
                ChargeLevel = 0f;
            }
            return;
        }
        ...
        void pickUpObject(GameObject obj){
            if (obj.GetComponent<Rigidbody>() != null){
                heldObject = obj;
                heldObject.transform.SetParent(cameraTransform.transform);
