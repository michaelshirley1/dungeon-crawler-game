	...
	public abstract class AbstractCombinable : MonoBehaviour {
		...
		public virtual void ClearCache(bool force = false) {
			...
			if (!isCached || force) {
				...
				if (cache.isSkinnedMesh) {
					...
					if (!isCached) {
						cache.blendShape.liveSync = new List<string>(cache.blendShape.enabled
		...
		private void Optimize() {
			if (cache.isSkinnedMesh && !cache.IsOptimized) {
				...
				(cache.mesh, cache.Bones) =
					cache.skinnedMeshRenderer.sharedMesh.Optimize(cache.skinnedMeshRenderer.bones);
				cache.IsOptimized = true;
			}
		}
	}
